...
void insertFigure(Node *node, void *figure, char type) {
    if (node->figuresCount < 10 && !node->reachedLimit) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // Iterate through the figures and insert them into the children nodes
            for (int j = 0; j < node->figuresCount; j++) {
                ...
                if (x >= child_x_min && x <= child_x_max && y >= child_y_min && y <= child_y_max) {
            ...
            // Insert the new figure into the appropriate child node
            float x, y;
...
void findOverlaps(Node *node, Node *root, pairList *pairs) {
    ...
    if(root == node) {
        ...
        for(int i = 0; i < 4; i++) {
            if(root->children != NULL && areNeighbors(node, root->children[i])) {
                if(areOverlapped(root->children[i]->figures[i], node->figures[i], root->children[i]->types[i], node->types[i]))
                    addToPairList(pairs, root->children[i]->figures[i], node->figures[i]);

                findOverlaps(node, root->children[i], pairs);
            }
        }
    }
...
void printOverlaps(Node *node, Node *root, pairList *pairs) {
    findOverlaps(node, root, pairs);
    if(pairs->pairsCount == 0)
...
int main() {
    ...
    figuresList->figures = (void **)malloc(sizeof(void *) * 15);
    figuresList->types = (char *)malloc(sizeof(char ) * 15);
    ...
    pairList *overlaps = (pairList *)malloc(sizeof(pairList));

    printOverlaps(root->children[0], root, overlaps);
    printOverlaps(root->children[1]->children[3], root, overlaps);
    printOverlaps(root->children[1]->children[2], root, overlaps);
    printOverlaps(root->children[1]->children[1], root, overlaps);
