...
void topologicalSort(int vertex, Graph *graph, int *visited, int *result, int *idx)
{
    visited[vertex] = 1;
    VertexNode *it = graph->adjList[vertex];
    while (it != NULL)
    {
        if (!visited[it->val])
        {
            topologicalSort(it->val, graph, visited, result, idx);
        }
        it = it->next;
    }
