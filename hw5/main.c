...
void AStar(int startRow, int startCol, int endRow, int endCol) {
    ...
    while (pq->size > 0) {
        ...
        int currentRow = (int)current->data / SIZE;
        int currentCol = (int)current->data % SIZE;
        free(current);
        ...
        for (int k = 0; k < 4; k++) {
            int newRow = currentRow + moves[k][0];
            int newCol = currentCol + moves[k][1];

            if (newRow >= 0 && newRow < SIZE && newCol >= 0 && newCol < SIZE && graph[newRow][newCol] != '#' && !visited[newRow][newCol]) {
                float newDist = dist[currentRow][currentCol] + weights2[newRow][newCol];
        ...
        if (bestRow != -1 && bestCol != -1) {
            dist[bestRow][bestCol] = dist[currentRow][currentCol] + weights2[bestRow][bestCol];
            parents[bestRow][bestCol] = currentRow * SIZE + currentCol;
...
void Dijkstra(int startRow, int startCol, int endRow, int endCol) {
    ...
    for (int count = 0; count < SIZE * SIZE; count++) {
        ...
        if(graph[minRow][minCol] != 'S' && graph[minRow][minCol] != 'E') {
            new[minRow][minCol] = '.';
            pathVisualiser(new);
        }
        //printf("Min: %d %d\n", minRow, minCol);
