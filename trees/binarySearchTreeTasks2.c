...
/*Колко на брой могат да бъдат минимално и максимално броя върхове на heap с височина h? Min: 2^(n-1), Max: предишният брой, умножен по 2, + 1;
Къде може да бъде минималната стойност в един heap ако в него има само уникални елементи? А ако може да има повторения? Във втората половина, 
където стойностите нямат деца. Иначе, ако се повтаря най-малкият елемент, и той може да си е дете, тайа че може да е малко по-назад в масива.
Този масив heap ли е -   33; 19; 20; 15; 13; 10; 2; 13; 16; 12? Не, 16 > 15, а 15 е родител в случая.
Binary Search дърветата имат интересното свойство, че ако вземем някой път от корена до някое листо и разделим върховете на 3 групи. 
А - върховете от ляво на този път, B - върховете на самия път и C - върховете от дясно на пътя то се получава следното. 
Ако вземем по един случаен връх от трите групи, съответно a, b, c, то a < b < c. Да, ама всъщност това не е вярно. Покажете, че не е вярно с контрапример.
Дървото ми tree изглежда така:
                   9
                /     \
               7       15
              / \     / \
             6   8   13  17
                     /
                    11
Ако избера като път 9-15-13, в група A ще са 7, 6, 8 и 11, в група B: 9, 15, 13, и в група C само 17.
в такъв случай, ако за a взема 11, то 11 < 9 < 17 е невярно.
*/
